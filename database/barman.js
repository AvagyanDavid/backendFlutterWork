    ...
    fs.rename(req.file.path, `${req.file.destination}${newFileName}`, (error) => {
        if (error) {
            ...
            db.query(`INSERT INTO BarmanEnd_StripBarsukKazan (Date,Time,Alcogol,Nonalcogol,Tobacco,Comment_Application,CommentDirector_Application,FillOutReport,Comment_FillOutReport,
                ...
                cleanlinessWorkplace, currentPath['cleanlinessWorkplaceFile'], req.body.commentCleanlinessWorkplace, req.body.commentDirectorCleanlinessWorkplace,idUser],
                (err) => {
                    if (err, otvet) {
                        console.log(err);
...
export const ShowBarmanEnd = (req, res) => {
    db.query({
        ...
        Users.Login, Users_idUsers from Users, BarmanEnd_StripBarsukKazan where BarmanEnd_StripBarsukKazan.Date = '${req.body.Date}' AND Users.idUsers = BarmanEnd_StripBarsukKazan.Users_idUsers;`
    }, (error, otvet) => {
        if (error) {
            console.log(error);
        } else {
...
export const UpdateBarmanEnd = (req, res) => {
    db.query(`update BarmanEnd_StripBarsukKazan set CommentDirector_Application = ?, CommentDirector_FillOutReport = ?, CommentDirector_CloseShift = ?, CommentDirector_CleanlinessWorkplace = ? where Users_idUsers = ? and Date = ?`,
